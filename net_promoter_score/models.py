...
def score_group(score):
    ...
    if score == -1:
        return PromoterScore.GROUP_UNKNOWN
    elif score < 7:
        return PromoterScore.GROUP_DETRACTOR
    elif score < 9:
        return PromoterScore.GROUP_NEUTRAL
    else:
        return PromoterScore.GROUP_PROMOTER


class PromoterScoreQuerySet(models.query.QuerySet):

    """Custom PromoterScore queryset - calculates the NPS."""

    def promoters(self):
        """Filter queryset by promoters."""
        return self.filter(group=PromoterScore.GROUP_PROMOTER)

    def detractors(self):
        """Filter queryset by detractors."""
        return self.filter(group=PromoterScore.GROUP_DETRACTOR)

    def neutrals(self):
        """Filter queryset by neutrals."""
        return self.filter(group=PromoterScore.GROUP_NEUTRAL)

    def net_promoter_score(self):
        """Calculate the NPS score from a set of PromoterScore objects.
    ...
    def most_recent_user_score(self, user):
        """Return the most recent user PromoterScore."""
...
class PromoterScore(models.Model):
    ...
    objects = PromoterScoreQuerySet.as_manager()
    ...
    def __repr__(self):
        return "<PromoterScore: id=%s user=%s score=%s>" % (
